...
func (r *JobRuntime) Run(ctx context.Context, destination string) error {
	for _, provider := range r.providers {
		...
		if err != nil {
			errorreporter.ReportError(err, "msg", "bad provider", "provider", provider)
			continue
		}
		for job := range js {
			if !r.Match(job) {
			...
			if err := os.MkdirAll(download_dir, 0750); err != nil {
				return err
			}
			if err := job.Download(jobCtx, download_dir); err != nil {
				bar.Complete(err)
				errorreporter.ReportError(err, "msg", "download error", "url", u, "download_dir", download_dir)
				continue
			}
			bar.Complete(nil)
		}
